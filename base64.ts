...
(function () {
	...
	Base64.number_hash = function (str: string, requiredLength?): string {
	...
	Base64.hmac = function (message: string, key: string): string {
		key = String(key);
		var k: Uint8Array;
		if (key.length > 32) k = Base64.hash(key, true);
		else {
	...
	Base64.write = function (str: string, key?: string): string {
		if (str === null) return "";
		str = String(str);
		str = convertTo(str);
		if (key) {
			var a, b: string[] = [], c = charset, d = Base64.hash(key), e, f = c + c + c + c + c;
	...
	Base64.read = function (str: string, key?: string): string | null {
		if (str === null) return "";
		str = String(str);
		if (key && /^d/.test(str)) {
			str = str.replace(/^d/, "");
			var a = str.length, b: string[] = [], c = charset, d = Base64.hash(key), e, f = c + c + c + c + c, g, h = String(d).length, i = c.length;
	...
	Base64.write_and_verify = function (str: string, key?: string): string {
	...
	Base64.createPublicKey = function (myPrivateKey: string): string {
	 ...
	 */
	Base64.createUserKey = function (userPublicKey: string, masterKey: string): string { return Base64.write("key:" + masterKey, getSessionKey(masterKey, userPublicKey)); };

	/*same as Base64.createUserKey, but needs the user's private key to verify*/
	Base64.createUserKey_and_verify = function (userPrivateKey: string, masterKey: string): string {
		//create public keys
	 ...
	 */
	Base64.readUserKey = function (myPrivateKey: string, userKey: string, mainPublicKey: string): string | false {
	 ...
	 */
	Base64.share = function (str: string, myPrivateKey: string, theirPublicKeys: string[], expires?: number | false): Base64ShareObj {
	...
	*/
	Base64.readShared = function (obj: Base64ShareObj, myPrivateKey: string): Base64ReadObj {
		var key,
