...
(function () {
	...
	Base64.getSessionKey = function (privateKey, publicKey) { return bigInt(publicKey).modPow(Base64.number_hash(privateKey, 100), prime).toString(); };
	 ...
	 */
	Base64.createUserKey = function (userPublicKey, masterKey) { return Base64.write("key:" + masterKey, Base64.getSessionKey(masterKey, userPublicKey)); };
	...
	Base64.createUserKey_and_verify = function (userPrivateKey, masterKey) {
		...
		var usersSessionKey = Base64.getSessionKey(userPrivateKey, mainPublicKey),
			mainSessionKey = Base64.getSessionKey(masterKey, userPublicKey);
	...
	Base64.readUserKey = function (myPrivateKey, userKey, mainPublicKey) {
		var key = Base64.read(userKey, Base64.getSessionKey(myPrivateKey, mainPublicKey));
	...
	*/
	Base64.readShared = function (obj, myPrivateKey) {
		var key,
			...
			data = null,
			hash = null;
		if (obj.Version === Base64.Version || obj.Version === 1.0) {
			if (obj.UserKeys && obj.Compressed) {
				var members = JSON && JSON.parse(Base64.read(obj.UserKeys)) || $.parseJSON(Base64.read(obj.UserKeys));
		...
		return { "type": type, "message": message, "data": data, "hash": hash };
	};
})();